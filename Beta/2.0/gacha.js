...
class BotGacha {
	...
	player_join(message, user) {
		...
		if (!this.players[user.id]) {
			...
			return message.channel.send(user.username + ` has registered!`);
		} else return message.channel.send(`You already been registered!`);
	}
	...
	async multi_pull(message, banner, rigged) {
		...
		if (!this.players[user.id])
			return message.channel.send(`Please use !register to register!`);
	...
	reset_pity(message) {
		...
		return message.channel.send(`Your counter has been reset!`);
	}
}

...
let spb = 'https://raw.githubusercontent.com/Rutheon/PaiB/main/File/Img/SPbanner/IMG_0088.png';
...
function set_pity(message, user, banner, num, featured) {
	...
	return console.log(
		`${user.username} - set pity ${Number(num)} / ${player.featured_5[0]}!`
	);
}
...
module.exports = {
	...
	utilisation: '{prefix}cb [Number] [Y]',
	...
	async execute(client, message, args) {
		...
		try {
			...
			let scE = new Discord.MessageEmbed()
				//.setTimestamp()
				.setFooter(
					message.author.tag + ' | Character Banner',
					message.author.avatarURL({ dynamic: true, format: 'jpg', size: 2048 })
				);

			/*-- admin --*/
			if (msg.author.id == client.config.author && cmd == '_cb admin') {
			...
			/*-- Set Pity --*/
			if (args[0]) {
				set_pity(msg, msg.author, 'C', args[0], args[1]);
			}

			/*-- Main Screen --*/
			scE.setTitle(`Character Banner: 정토 좆는 천하인`);
			scE.setColor('GREEN');
			scE.setImage(spb);
			scE.setDescription(
				'1️⃣ - 1 연차\n🔟 - 10 연차\n✳️ - 가챠 목록 보기\n✅ - 종료 및 결과 확인'
			...
			collector.on('collect', async (reaction, user) => {
				try {
					if (reaction.emoji.name === '1️⃣') {
						...
						let p1 = multi_pull(msg, 0, 0);
						let pst1 = playerGI[user.id];
						if (pst1.featured_5[0] == true) {
							var ft1 = '확정';
						} else if (pst1.featured_5[0] == false) {
							var ft1 = '픽뚫';
						}
						if (pst1.featured_4[0] == true) {
							var ft41 = '확정';
						} else if (pst1.featured_4[0] == false) {
							var ft41 = '픽뚫';
						}
						scE.setTitle(
							`1️⃣ 연차 결과 {총: ${pst1.pulls[0]}스택}\n[ 4성 ${ft41} \u279C ${
								pst1.pity_4[0]
							}스택 | 5성 ${ft1} \u279C ${pst1.pity_5[0]}스택 ]`
						);
						if (p1.star.includes('★★★★★')) {
						...
						reaction.message.reactions.removeAll();
						let p2 = multi_pull(msg, 0, 1);
						let pst2 = playerGI[user.id];
						if (pst2.featured_5[0] == true) {
							var ft2 = '확정';
						} else if (pst2.featured_5[0] == false) {
							var ft2 = '픽뚫';
						}
						if (pst2.featured_4[0] == true) {
							var ft42 = '확정';
						} else if (pst2.featured_4[0] == false) {
							var ft42 = '픽뚫';
						}
						scE.setTitle(
							`🔟 연차 결과 {총: ${pst2.pulls[0]}스택}\n[ 4성 ${ft42} \u279C ${
								pst2.pity_4[0]
							}스택 | 5성 ${ft2} \u279C ${pst2.pity_5[0]}스택 ]`
						);
						if (p2.star.includes('★★★★★')) {
							scE.setImage(multi5);
						} else {
